...
int main(int argc, char **argv)
{
   ...
   QX_PRINTLN("[+] loading circuit from '" << file_name << "' ...");
   ...
   QX_PRINTLN("[+] creating quantum register of " << qcp.qubits() << " qubits... ");
   try {
	   reg = new qx::qu_register(qcp.qubits());
   } catch(std::bad_alloc& exception) {
	   QX_PRINTLN("[!] not enough memory, aborting");
	   //xpu::clean();
	   return -1;
   } catch(std::exception& exception) {
	   QX_PRINTLN("[!] unexpected exception (" << exception.what() << "), aborting");
	   //xpu::clean();
	   return -1;
   }
   ...
   {
      QX_PRINTLN("[+] generating noisy circuits...");
      ...
      {
         QX_PRINTLN("[>] processing circuit '" << perfect_circuits[i]->id() << "'...");
	 ...
	 if (iterations > 1)
	 {
	    for (uint32_t it=0; it<iterations; ++it)
	    {
	       ...
	       QX_PRINTLN("[+] noisy circuit : " << std::hex << noisy_c);
	 ...
	 else
	 {
      ...
      QX_PRINTLN("[+] total errors injected in all circuits : " << total_errors);
